...
describe("Contract 'FundsDistributor'", async () => {
    ...
    before(async () => {
        [deployer, pauser, upgrader, user, attacker, random] = await ethers.getSigners();
    ...
    async function createSignature(signer: HardhatEthersSigner, amount: number, nonce: number, chainId: number): Promise<string> {
    ...
    async function deployContracts(): Promise<{ token: Contract, distributor: Contract }> {
    ...
    describe("Functions 'initialize()' and '_authorizeUpgrade'", async () => {
        ...
        it("Initializer is reverted if called second time", async () => {
            ...
            await expect(distributor.initialize(pauser.address, upgrader.address, deployer.address, random.address))
    ...
    describe("Function 'claimReward()'", async () => {
        it("Executes as expected", async () => {
            ...
            const signature = await createSignature(user, REWARD_AMOUNT, startingNonce, HARDHAT_CHAIN_ID);

            // user, amount, nonce, chainId, signature
            expect(await distributor.verifySignature(user.address, REWARD_AMOUNT, startingNonce, HARDHAT_CHAIN_ID, signature)).to.eq(true);

            const tx = await getTx(distributorConnectedToUser.claimReward(REWARD_AMOUNT, startingNonce, signature))

            // check event and balance update
            expect(tx).to.emit(distributor, EVENT_NAME_REWARD_PAID).withArgs(user.address, REWARD_AMOUNT)
        ...
        it("Is reverted if nonce is invalid", async () => {
            const { distributor } = await loadFixture(deployContracts);
            const signature = await createSignature(user, REWARD_AMOUNT, 322, HARDHAT_CHAIN_ID);
        ...
        it("Is reverted if the caller is not the signer", async () => {
            const { distributor } = await loadFixture(deployContracts);
            const signature = await createSignature(user, REWARD_AMOUNT, 0, HARDHAT_CHAIN_ID);
            const distributorConnectedToAttacker = distributor.connect(attacker) as Contract;
        ...
        it("Is reverted if the amount is not the signed one", async () => {
            const { distributor } = await loadFixture(deployContracts);
            const signature = await createSignature(deployer, REWARD_AMOUNT, 0, HARDHAT_CHAIN_ID);
